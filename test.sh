...
function check_bluetooth {
    ...
    if [ -z "$bluetooth_status" ] || echo "$bluetooth_status" | grep -q "yes"; then
...
function pair_device {
    ...
    while [ "$retry_count" -lt "$max_retries" ]; do
        ...
        if [ "$use_pin" == "y" ]; then
            ...
            result=$(bluetoothctl pair "$device_mac" "$pin_code")
        else
            result=$(bluetoothctl pair "$device_mac")
        fi
